            ...
            <template v-if="device === 'mobile'">
              <div class="item">
                ...
                <template slot-scope="scope">
                  <router-link v-if="scope.$index < tableData.length - 2" :to="`/giftReport/giftReport/${scope.row.agentId}?searchTime=${searchTime}`">
              ...
              <div class="item">
                ...
                <span class="content">{{ item.giftRate }}</span>
              </div>
              <div class="item">
                <span class="header">{{ $t('__totalAmount') }}</span>
                <span :class="{positive: item.netPL >= 0, negative: item.netPL < 0}">{{ item.netPLLabel }}</span>
              </div>
              <div class="item">
...
<script>
import { agentGiftReportSearch } from '@/api/giftReport/agent'
...
export default {
  ...
  computed: {
    ...
    agentWidth() {
      return this.calculateWidth(this.$t('__agent'), 'agent', 8, 35) + 'px'
    }
  },
  methods: {
    ...
    onSubmit() {
      ...
      agentGiftReportSearch(data).then((res) => {
