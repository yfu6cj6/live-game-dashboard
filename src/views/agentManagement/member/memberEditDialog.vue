    ...
    <template v-if="device==='mobile'">
      <div v-loading="dialogLoading" class="popup-page">
        <div class="w-100 pt-3 pb-3">
          ...
          >
            <el-step v-if="hasStep('memberInfo')" />
        ...
        <div class="form-step-content flex-column flex-fill">
              ...
              <div class="step-content">
                ...
                <div class="el-form-item small el-form-item--feedback el-form-item--small">
                  <div class="el-form-item__content">
                    <div class="form-item-inner">
                      <div class="label-group">
                        <label class="form-item-label w-50">{{ $t('__agentBalance') }}</label>
                        <label
                ...
                <div class="el-form-item small el-form-item--feedback el-form-item--small">
                  <div class="el-form-item__content">
                    <div class="form-item-inner">
                      <div class="label-group">
                        <label class="form-item-label w-50">{{ $t('__memberAccount') }}</label>
                ...
                <div class="el-form-item small el-form-item--feedback el-form-item--small">
                  <div class="el-form-item__content">
                    <div class="form-item-inner">
                      <div class="label-group">
                        <label class="form-item-label w-50">{{ $t('__memberBalance') }}</label>
          ...
          >
            <form class="el-form flex-column flex-fill el-form--label-left">
              <label class="form-item-title">{{ `${$t('__confirm')}${$t('__member')}${$t('__info')}` }}</label>
              <div class="preview">
...
export default {
  ...
  props: {
    ...
    'stepEnum': {
      ...
      require: true,
      default() {
        return Object.freeze({ 'memberInfo': 0, 'rate': 1, 'limit': 2, 'balanceConfig': 3, 'confirm': 4 })
      }
    }
  },
  data: function() {
    return {
      ...
      step2: {
        ...
        max_win_amount_limit: {
          hasError: false,
          isSuccess: false
        },
        max_lose_amount_limit: {
          hasError: false,
          isSuccess: false
        }
      },
  ...
  computed: {
    ...
    maxWinAmountLimitStr() {
      return numberFormat(this.form.max_win_amount_limit)
    },
    maxLoseAmountLimitStr() {
      return numberFormat(this.form.max_lose_amount_limit)
    },
    existHandicaps() {
  ...
  methods: {
    ...
    specialInputChange(attributes) {
      switch (attributes) {
        ...
        case 'max_win_amount_limit': {
          // 沒有輸入
          const noValue = (!this.form.max_win_amount_limit && this.form.max_win_amount_limit !== 0)
          if (noValue || this.form.max_win_amount_limit < 0) {
            this.step2.max_win_amount_limit.isSuccess = false
            this.step2.max_win_amount_limit.hasError = true
            return false
          } else {
            this.step2.max_win_amount_limit.isSuccess = true
            this.step2.max_win_amount_limit.hasError = false
            return true
          }
        }
        case 'max_lose_amount_limit': {
          // 沒有輸入
          const noValue = (!this.form.max_lose_amount_limit && this.form.max_lose_amount_limit !== 0)
          if (noValue || this.form.max_lose_amount_limit < 0) {
            this.step2.max_lose_amount_limit.isSuccess = false
            this.step2.max_lose_amount_limit.hasError = true
            return false
          } else {
            this.step2.max_lose_amount_limit.isSuccess = true
            this.step2.max_lose_amount_limit.hasError = false
            return true
          }
        }
        case 'balance': {
    ...
    onNextBtnClick() {
      ...
      if (this.curIndex === this.stepEnum.memberInfo) {
        ...
        if (this.operationType === this.operationEnum.create && this.visible) {
          if (!this.nameChange(this.step1.name, this.form.name)) {
            success = false
          }
        ...
        if (!this.specialInputChange('max_win_amount_limit')) {
          success = false
        }
        if (!this.specialInputChange('max_lose_amount_limit')) {
          success = false
        }
        if (!success) {
